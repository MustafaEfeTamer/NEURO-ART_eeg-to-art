...
class Record():
    ...
    def wait(self, record_duration_s):
        ...
        while length < record_duration_s:
            print(f"recording at {length} s  (durdurmak için Q bas)")
            time.sleep(1)
            length += 1

            # Q'ya basılırsa durdur
            if keyboard.is_pressed('q'):
                print("Q algılandı, kayıt hemen durduruluyor...")
                break
    ...
    def on_stop_record_done(self, *args, **kwargs):
        ...
        title = data['title']
        print('The record has been stopped: recordId: {0}, title: {1}, startTime: {2}, endTime: {3}'.format(record_id,
                                                                                                            title,
                                                                                                            start_time,
                                                                                                            end_time))
    ...
    def on_warn_record_post_processing_done(self, *args, **kwargs):
        ...
        print('on_warn_record_post_processing_done: The record', record_id,
              'has been post-processed. Now, you can export the record')
...
def main():
    ...
    r.record_title = 'halil-ayak'  # required param and can not be empty
    r.record_description = 'halil-ayak'  # optional param

    # input params for export_record. Please see on_warn_cortex_stop_all_sub()
    r.record_export_folder = 'C:/users/kou/desktop/HALIL'  # your place to export, you should have write permission, example on desktop
    r.record_export_data_types = ['EEG', 'MOTION', 'PM', 'BP']
    r.record_export_format = 'CSV'
    r.record_export_version = 'V2'

    record_duration_s = 300  # duration for recording in this example. It is not input param of create_record
